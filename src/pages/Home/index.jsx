...
const index = () => {
    ...
    if (syms.length < 4) {
      if (!syms.includes(index)) {
        if (idVariable == dep_id) {
          ...
          if (idVariable != 'all') {
            var text =
              'Can only choose symptoms from one department\nDo you want to change department?';
            if (confirm(text)) {
  ...
  const getSymptomsName = () => {
    const fitler_syms = symptoms.filter(
  ...
  for (let i = 0; i < size; ++i) {
    data[Object.keys(dep_symptoms)[i]] = dep_symptoms[
      Object.keys(dep_symptoms)[i]
    ].data.map(({ dep_id, index, name, img }) => {
      const isChoose = syms.includes(index) && syms.length < 4;
      // console.log(index, isChoose);
      return (
        <div
          onClick={() => onChoose(dep_id, index)}
          key={index}
          props={dep_id}
        ...
        >
          <div
            style={{
              backgroundColor: isChoose ? color.choose : color.not_choose,
            }}
    ...
    return (
      <div key={id}>
        <div className={styles.root}>
          ...
          <div className={styles.root}>{data[id]}</div>
        </div>
      </div>
    );
  });
  ...
  return (
    <>
      <div className={styles.head} style={{ position: 'relative' }}>
        <div className={styles.side}>
          <div className={styles.title}>
            <h1 className={styles.titleH1}>Consult top Doctors online</h1>
            <p className={styles.titlep}>We are partners with top hospitals</p>
          </div>
      ...
      <div className={styles.wrapper}>
        <div className={styles.root}>{data_render}</div>
      </div>
      ...
      <div className={styles.chosen_sym}>
        <div className={styles.content} style={{ marginLeft: '100px' }}>
          ...
          <div className={styles.symptoms_wrap}>{chosen_symptoms}</div>
        </div>
